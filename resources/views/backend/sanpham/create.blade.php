...
<form action="{{route('admin.sanpham.store')}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group col-md-10">
        ...
        placeholder="Tên sản phẩm" value="{{ old('sp_ten') }}">
    ...
    <div class="form-group col-md-10">
        ...
        placeholder="Tên sản phẩm" value="{{ old('sp_hinhAnh') }}">
    ...
    <div class="form-group col-md-10">
        ...
        placeholder="Mô tả ngắn" value="{{ old('sp_moTa_ngan') }}">
    ...
    <div class="form-group col-md-10">
        ...
        placeholder="Giá nhập sản phẩm" value="{{ old('sp_giaNhap') }}">
    ...
    <div class="form-group col-md-10">
        ...
        placeholder="Giá bán sản phẩm" value="{{ old('sp_giaBan') }}">
    ...
    <div class="form-group col-md-10">
        ...
        placeholder="Số lượng" value="{{ old('sp_soLuong') }}">
    ...
    <div class="form-group col-md-4">
        ...
        <select id="sp_noiBat" name="sp_noiBat" class="form-control">
            <option value="1"  {{ old('sp_noiBat') == 1 ? "selected" : "" }}>Có</option>
            <option value="2"  {{ old('sp_noiBat') == 2 ? "selected" : "" }}>Không</option>
        </select>
    </div>
    <div class="form-group col-md-4">
        <label for="sp_moi">Sản phẩm mới</label>
        <select id="sp_moi" name="sp_moi" class="form-control">
            <option value="1"  {{ old('sp_moi') == 1 ? "selected" : "" }}>Có</option>
            <option value="2"  {{ old('sp_moi') == 2 ? "selected" : "" }}>Không</option>
        </select>
    </div>
    <div class="form-group col-md-4">
        ...
        <select name="l_ma" id="l_ma" class="form-control">Loại
            @foreach($dsLoai as $loai)
                <option value="{{$loai->l_ma}}" {{ old('l_ma') == $loai->l_ma ? "selected" : "" }}>{{$loai->l_ten}}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group col-md-4">
        ...
        <select name="ncc_ma" id="ncc_ma" class="form-control">Loại
            @foreach($dsNcc as $ncc)
                <option value="{{$ncc->ncc_ma}}" {{ old('ncc_ma') == $ncc->ncc_ma ? "selected" : "" }}>{{$ncc->ncc_ten}}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group col-md-4">
    <label for="inputState">Trạng thái</label>
      <select id="inputState" name="sp_trangThai" class="form-control">
        ...
        <option value="2"   {{ old('sp_trangThai') == 2 ? "selected" : "" }} selected>Khả dụng</option>
      </select>
  </div>                   
